...
**项目类型：** 数据库课程设计  
**系统形态：** 微信小程序 + PC 管理端 + Flask 服务 + MySQL  
**提交截止：** 2026 年 7 月 1 日 24:00  
**提交格式：** PDF（每组一位同学提交）  
**GitHub 仓库：** 尚未配置。提交前必须创建 Public 仓库并将实际链接写入本报告。  

## 摘要

本项目面向高校课堂考勤场景，设计并实现一套集签到、请假、统计与后台管理于一体的课堂扫码签到系统。教师可以创建签到场次、生成二维码、设置签到时间及地理位置范围；学生通过微信小程序扫码，系统依次校验二维码、场次状态、学生身份、选课关系、重复签到、时间窗口与定位距离，并记录正常、迟到、缺勤、无效或请假等状态。教师结束场次后，系统自动为未产生有效考勤记录的在选学生补录缺勤。学生可针对具体场次提交请假申请，教师审批后系统在事务中同步更新请假申请和考勤记录。系统还提供学生、教学班、课程、院系、教师、时间趋势及异常预警等统计功能，并通过小程序和 PC 管理端进行展示。

系统采用 MySQL 保存业务数据，以 Flask 提供核心业务 API 和统计 API；微信小程序承担学生与教师的移动端操作，PC 管理端承担数据维护和可视化分析。项目重点解决了重复签到、数据一致性、定位校验、缺勤自动补录、请假与统计口径衔接等问题。

**关键词：** 课堂考勤；微信小程序；MySQL；Flask；二维码；请假审批；数据统计

## 1 项目概述

### 1.1 项目背景

传统课堂点名耗时较长，纸质签到存在代签、统计困难和数据无法复用等问题。普通二维码签到虽然提高了效率，但仍需要处理二维码转发、重复签到、非选课学生签到、超时签到、远距离签到及请假与缺勤冲突等业务问题。因此，本项目以数据库设计为核心，将签到业务规则、请假审批和统计分析整合为一个完整系统。

### 1.2 建设目标

- 建立教师、课程、教学班、学生、选课、签到场次、考勤记录和请假申请之间的完整数据模型。
- 实现教师创建签到、学生扫码签到、教师结束签到和系统自动补录缺勤的闭环。
- 通过事务、行锁、唯一约束和状态校验保证数据一致性。
- 提供学生请假、教师审批、缺勤转请假及请假统计处理。
- 提供面向学生、教师和管理员的多维统计与数据管理功能。
- 提供可复现的安装脚本、演示数据和使用说明。

### 1.3 系统边界

系统当前定位为课程设计和本地部署版本。用户身份由小程序本地缓存保存，尚未接入学校统一身份认证；后端通过本地 HTTP 服务提供接口，真机部署时需要配置局域网地址或 HTTPS 合法域名。请假支持文字原因与审批备注，暂不包含附件上传和消息推送。

### 1.4 期中后主要改动

1. 完成教师创建签到、二维码生成、学生扫码、定位校验和缺勤自动补录的完整流程。
2. 增加教师课程动态加载、历史场次、二维码保存及签到记录查询。
3. 将统计功能扩展为独立 Flask 服务，增加学生、班级、院系、教师、趋势和异常预警统计。
4. 完成 PC 管理端的学生、教学班、考勤记录和统计看板。
5. 新增学生请假、教师审批、缺勤转请假及统计口径调整。
6. 增加 `demo_data.sql` 和 `demo_simulate.py`，支持批量生成答辩展示数据。
...
| 演示支持 | `demo_data.sql`、`code/demo_simulate.py` | 基础演示数据和历史考勤模拟 |
...
- 学生端、教师端和统计页 JavaScript 通过 Node `--check`。
- Flask 核心应用已注册 37 条路由，未发现重复端点。
- 已完成 MySQL 实库只读联调：教师、课程、签到场次、教学班和教学班学生名单接口均可返回数据。
- 请假 Blueprint、管理 Blueprint 与核心签到路由能够由 `app.py` 统一加载。

### 9.4 测试结论与剩余风险

核心业务链路和静态检查已经完成。生产化仍需补充统一身份认证、权限令牌、HTTPS、接口自动化测试和并发压力测试。当前管理端依赖 CDN，离线环境下可能无法加载 Vue、Element Plus 和 ECharts。
...
`attendance_system.sql` 已包含 `student.gender` 字段；`demo_data.sql` 仍会兼容检查旧库，然后创建 T001 教师、C001/C002 课程、CL001/CL002 教学班和 50 名学生。

### 10.3 配置数据库连接

打开 `code/db.py`，将 `password` 修改为本机 MySQL 密码。两个 Flask 服务都通过该文件连接 `attendance_system` 数据库。
...
```text
demo_data.sql + code/demo_simulate.py
```

`demo_data.sql` 创建完整基础数据，`demo_simulate.py` 为 CL001 和 CL002 各生成 5 周历史签到场次及考勤记录。该组合已经覆盖 `demo_seed.sql` 的用途，因此 `demo_seed.sql` 不再是必需文件，可以删除或保留为早期简化示例，但文档和最终演示均不再使用它。
...
执行前必须先运行 `attendance_system.sql`、`leave_feature.sql`（旧库需要）和 `demo_data.sql`。

> 重要警告：`demo_simulate.py` 会清空 `leave_request`、`attendance_record` 和 `attendance_session` 中的现有数据，然后重新生成模拟数据。它只能用于答辩演示数据库；真实数据必须先备份，不能在生产数据库执行。
...
1. 教师 T001 对 CL002 创建新签到并展示二维码。
2. 学生 2024001 扫码并查看出勤记录。
3. 学生 2024030 对新场次提交请假。
4. 教师 T001 批准申请。
5. 学生刷新并确认请假状态。
6. 教师关闭场次并展示统计看板。
...
| `code/demo_simulate.py` | 历史考勤模拟脚本，会清空旧考勤数据 |
| `admin_web/index.html` | PC 管理端单页应用 |
| `attendance_system.sql` | 全量数据库建表脚本 |
| `leave_feature.sql` | 旧数据库请假功能迁移脚本 |
| `demo_data.sql` | 最终答辩基础数据脚本 |

## 14 系统不足与改进方向

1. 接入学校统一身份认证，后端增加 Token 鉴权和角色权限控制。
2. 为二维码增加更短有效期、动态刷新或设备指纹，提高抗转发能力。
3. 请假增加图片或证明附件、消息通知和批量审批。
4. 将管理端依赖改为本地构建，避免 CDN 对网络的依赖。
5. 使用 HTTPS、环境变量和独立数据库账户，避免明文保存 root 密码。
6. 增加 pytest 接口测试、前端自动化测试和并发压力测试。
7. 将核心业务和统计服务容器化，并增加日志、监控和定时任务。

## 15 总结

本项目从数据库概念模型出发，完成了课堂签到、定位校验、缺勤补录、请假审批、考勤修正和统计分析的完整实现。系统通过外键和唯一约束维护基础关系，通过事务和行锁处理并发与一致性，通过独立统计服务提供多维分析。当前版本已经满足课程设计的需求分析、数据库设计、功能设计、模块实现、安装使用和演示测试要求，并保留了进一步向真实校园系统演进的空间。

## 附录 A 提交清单

- 项目代码：整个 `D_leave_project` 目录。
- 全量数据库脚本：`attendance_system.sql`。
- 旧库迁移脚本：`leave_feature.sql`。
- 演示数据脚本：`demo_data.sql`。
- 历史模拟脚本：`code/demo_simulate.py`。
- 项目报告：`output/pdf/课堂扫码签到与请假管理系统-期末项目报告.pdf`。
- 安装和使用说明：项目文档第 10、11 章。
- 答辩演示说明：项目文档第 12 章和演示彩排手册。

## 附录 B 交付前检查

1. 数据库密码未出现在 PPT、截图和公开提交说明中。
2. `app.py` 和 `query.py` 能分别在 5000、8080 端口启动。
3. 微信开发者工具能导入项目并完成编译。
4. T001、2024001 和 2024030 可完成答辩演示流程。
5. 项目文档中的文件名、接口和状态值与最终代码一致。
6. `demo_simulate.py` 只在演示数据库中运行，并已备份需要保留的数据。
7. GitHub 仓库已设为 Public，README 可在仓库首页查看，报告中的仓库链接可正常访问。
